...
def test_links_provider_filters_small_preview_urls_before_aria2(tmp_path, monkeypatch):
    provider = LinksProvider()
    seen = {}
...
def test_extract_tries_no_password_before_password_candidates(tmp_path, monkeypatch):
    input_dir = tmp_path / "input"
    output_dir = tmp_path / "output"
    input_dir.mkdir()
    ...
    assert "Archive password candidates: 2; trying no-password first, then password candidates" in job.logs
    assert "Trying 7z without password" in job.logs
    assert "Trying 7z with password #1" in job.logs
...
class TransferJobTests(TestCase):
    ...
    def test_terabox_rename_api_error_is_upload_failed(self):
        class Response:
            ...
            def json(self):
                return {"errno": 12, "errmsg": "rename failed"}
